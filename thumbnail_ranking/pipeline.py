...
def rank_and_store_top_candidates(
    ...
    if not quality_metrics:
        ...
        print(f"\n  Current thresholds:")
        print(f"    brightness: {quality_thresholds.min_brightness} < x < {quality_thresholds.max_brightness}")
        print(f"    contrast: > {quality_thresholds.min_contrast}")
        print(f"    sharpness: > {quality_thresholds.min_sharpness}")
        print(f"    edge_density: > {quality_thresholds.min_edge_density}")
        raise ValueError(
    ...
    for i, ranked in enumerate(top_ranked, start=1):
        ...
        dest_filename = f"rank_{i}_score_{ranked.clip_score:.4f}_{ranked.metrics.filename}"
...
def select_best_thumbnail(
    ...
    top_ranked = rank_and_store_top_candidates(video_path, top_n=1, quality_thresholds=quality_thresholds)
